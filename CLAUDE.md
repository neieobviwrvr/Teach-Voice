...
- **Check for abuse/anomalies on `grade-answer`/`generate-questions`**: no automated alerting exists (deliberately not built yet — would need a monitoring service, a bigger decision Simon hasn't greenlit). Manual check via the Management API's `database/query` endpoint against `public.rate_limit_counters`, e.g. `select identity, request_count, window_start from public.rate_limit_counters where window_start > now() - interval '24 hours' order by request_count desc limit 20;` — a normal identity should never be anywhere near the per-hour caps (150 for grade-answer, 40 for generate-questions, see the `RATE_LIMIT_MAX_PER_HOUR` constants in each function); anything close to those, or an unusually high `global:all-guests` count, is worth investigating. Complement with a look at the OpenAI usage dashboard for cost spikes.
...
**Hard limits, deliberately tight for the current starting phase** (Simon called these explicit starting values, not a final ceiling — expect them to loosen later): max 2 folders per user (raised from 1 so Simon could test the top-left folder-picker dropdown on `FolderListView`), **unlimited subfolders per folder**, max 25 flashcards per subfolder, and — the actual brake now that subfolder count is unlimited — max 50 flashcards per folder *in total, summed across all of its subfolders*. Named constants in `Models.swift` (`maxFoldersPerUser`, `maxFlashcardsPerSubfolder`, `maxFlashcardsPerFolder`; `maxSubfoldersPerFolder` no longer exists, removed on purpose). Each is enforced *twice* — a Postgres trigger (`enforce_folder_limit` / `enforce_flashcard_limit`, cloud mode, see `supabase/migrations/0003_tighter_limits.sql`, `0005_raise_flashcard_limit.sql`, `0007_unlimited_subfolders.sql`, `0011_raise_folder_limit.sql`) and an equivalent check inside `LocalLibraryRepository` (guest mode) — keep both sides in sync if any of these limits change. `LibraryStore` also does a client-side pre-check for immediate UI feedback before ever calling the repository; for the folder-wide total this requires `LibraryStore.loadAllFlashcards(for:)` to have run first (loads every subfolder's cards, not just visited ones), otherwise the client-side number can undercount — the server/repository side remains the actual authority.
