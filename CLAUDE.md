...
**Hard limits, deliberately tight for the current starting phase** (Simon called these explicit starting values, not a final ceiling — expect them to loosen later): max 1 folder per user, **unlimited subfolders per folder**, max 25 flashcards per subfolder, and — the actual brake now that subfolder count is unlimited — max 50 flashcards per folder *in total, summed across all of its subfolders*. Named constants in `Models.swift` (`maxFoldersPerUser`, `maxFlashcardsPerSubfolder`, `maxFlashcardsPerFolder`; `maxSubfoldersPerFolder` no longer exists, removed on purpose). Each is enforced *twice* — a Postgres trigger (`enforce_folder_limit` / `enforce_flashcard_limit`, cloud mode, see `supabase/migrations/0003_tighter_limits.sql`, `0005_raise_flashcard_limit.sql`, `0007_unlimited_subfolders.sql`) and an equivalent check inside `LocalLibraryRepository` (guest mode) — keep both sides in sync if any of these limits change. `LibraryStore` also does a client-side pre-check for immediate UI feedback before ever calling the repository; for the folder-wide total this requires `LibraryStore.loadAllFlashcards(for:)` to have run first (loads every subfolder's cards, not just visited ones), otherwise the client-side number can undercount — the server/repository side remains the actual authority.
